...
src/index.js            진입점: fetch 핸들러, 라우트 분배, 미들웨어(세션·CSRF·보안헤더·에러로깅)
  └─ src/handlers/      도메인별 요청 처리 (session, viewer, document, rack, set, admin)
       ...
       └─ src/db.js     데이터 계층 배럴 → src/data/*  (D1 질의·변경, 검색, 감사로그)
            └─ 공용 리프 모듈: searchCore.js, utils.js, auth.js, security.js, config.js, routes.js, documentCsv.js
```
...
| `src/views/` | `layout.js`(페이지 셸·공용 조각) · `styles.js`(CSS) · `clientScript.js`(전역 클라이언트 JS) · 도메인별 페이지 |
| `src/data/` | `sqlShared.js`(공용 SQL 조각) · 도메인별 데이터 접근. SQL은 전부 바인딩 파라미터 |
