...
- **모바일 하단 탭**: 검색·문서고 도면·엑셀대장·폐기 중 권한이 있는 핵심 메뉴만 표시. 높이 48px, `--surface` + 상단 `--line`, 활성은 사이드바와 같은 색 문법.
- **엑셀 대장 동기화**: 파일 선택 → 구조 확인 → 전송 진행률 → 유형별 요약·before/after·제외 목록 검토 → 사유/승인 참조 입력 → 명시적 반영 순서를 유지한다. 파일 선택만으로 현재 대장을 바꾸지 않으며, 제외·폐기 해제는 위험 경고로 구분한다.
