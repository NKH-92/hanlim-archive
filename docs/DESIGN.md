...
- **모바일 하단 탭**: 검색·문서고 도면·리스트 동기화·문서 추가·폐기 중 권한이 있는 핵심 메뉴만 표시. 높이 48px, `--surface` + 상단 `--line`, 활성은 사이드바와 같은 색 문법.
- **리스트 동기화와 문서 추가**: `리스트 동기화`는 엑셀 전체 동기화만 제공하고, 수시 건별 등록은 별도 `문서 추가` 탭에서 처리한다. 문서 수정·개정·위치 이동·폐기는 `문서 관리`에서 처리한다.
...
- **문서고 도면**: 검색 화면과 분리된 전용 메뉴에서 전체 도면을 제공하고, 랙 선택 시 해당 위치 검색으로 이동한다. 문서 상세의 위치 도면과 랙 열·선반 표시는 접지 않고 기본 노출한다.
- **운영 히어로**: 검색·문서 위치·엑셀·폐기·운영 관리의 현재 작업과 실제 상태를 `--hero-bg` 면 하나로 요약한다. 장식용 통계나 read model에 없는 숫자는 만들지 않는다.
