...
const abandonedDays = Math.max(1, Math.min(Number(arg("--abandoned-days", "7")) || 7, 365));
if (!dbPath) {
...
const movementSnapshotMatch = hasColumn("document_movements", "snapshot_code")
  ? "m.snapshot_code = json_extract(a.details, '$.snapshotCode')"
  : `m.reason = json_extract(a.details, '$.applyReason')
       AND m.performed_by_username = a.actor_username
       AND ABS(strftime('%s', m.created_at) - strftime('%s', a.created_at)) <= 300`;

const report = {
  generatedAt: new Date().toISOString(),
  dbPath,
  options: { abandonedDays },
  findings: {
    currentIdentityDuplicates: all(`
      ...
      HAVING COUNT(*) > 1
    `),
    currentRequiredFieldMissing: all(`
      SELECT id AS document_id, document_number, revision_number, revision_date, disposal_due_year
      FROM documents
      WHERE sync_state = 'current'
        AND (revision_date IS NULL OR disposal_due_year IS NULL)
      ORDER BY id
    `),
    ...
    dateMinusOneDayCandidates: all(`
      SELECT
        ...
        json_extract(details, '$.snapshotCode') AS snapshot_code,
        COALESCE(
          json_extract(details, '$.before.values.revisionDate'),
          json_extract(details, '$.before.revisionDate')
        ) AS before_date,
        COALESCE(
    ...
    abandonedSnapshots: all(`
      ...
      WHERE status IN ('staging', 'ready')
        AND datetime(COALESCE(updated_at, created_at)) <= datetime('now', '-${abandonedDays} days')
      ORDER BY created_at
    `),
