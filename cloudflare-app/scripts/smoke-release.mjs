...
export async function runReleaseSmoke({ baseUrl, username, password, fetchImpl = fetch }) {
  const origin = String(baseUrl || "").replace(/\/$/, "");
  if (!origin || !username || !password) throw new Error("SMOKE_BASE_URL, SMOKE_USERNAME, SMOKE_PASSWORD가 필요합니다.");

  const health = await fetchImpl(`${origin}/healthz`, { redirect: "manual" });
  if (health.status !== 200 || !(await health.json()).ok) throw new Error("/healthz smoke 실패");
