...
import { documentsPage } from "../../views/documentViews.js";
...
export async function handleDocuments(request, env, session) {
  ...
  if (!parsed.text) {
    ...
    const totalDocuments = await getFastDocumentCount(env, filters);
    const window = await getDocumentPageWindow(env, filters, page, pageSize);
    const documents = window.items;
    const suggestions = buildSearchSuggestions(documents, 10);
    const knownOrVisibleCount = totalDocuments ?? ((page - 1) * pageSize + documents.length + (window.hasMore ? 1 : 0));
    const didYouMean = await resolveSearchOutcome(env, { ...search, filters, page }, knownOrVisibleCount);
    ...
    return documentsPage({
      ...
      pagination: {
        ...
        pageSize,
        totalDocuments,
        totalPages: totalDocuments === null ? null : Math.max(1, Math.ceil(totalDocuments / pageSize)),
