...
import { requireApplyDocumentSnapshots, requireManageDocuments } from "./permissionGuards.js";
...
function statusForSnapshotError(result) {
  ...
  switch (result.code) {
    ...
    case SNAPSHOT_ERROR_CODES.SNAPSHOT_INVALID_STATE:
    case SNAPSHOT_ERROR_CODES.SNAPSHOT_EXPORT_PROVENANCE_INVALID:
    ...
    case SNAPSHOT_ERROR_CODES.SNAPSHOT_REVISION_HISTORY_CONFLICT:
      return 409;
    case SNAPSHOT_ERROR_CODES.SNAPSHOT_FILE_TOO_LARGE:
    case SNAPSHOT_ERROR_CODES.SNAPSHOT_ZIP_UNSAFE:
      return 413;
...
export async function handleCreateDocumentSnapshot(request, env, session) {
  ...
  const result = await createDocumentSnapshot(env, {
    ...
    sourceSize: form.get("sourceSize"),
    ...
    backupConfirmed: form.get("backupConfirmed")
  }, session);
...
export async function handleDocumentSnapshotRoute(request, env, session, routeInfo) {
  ...
  if (request.method === "GET" && action === "details") {
    ...
    return documentSnapshotDetailPage({
      ...
      warnings,
      validationErrors: parseValidationErrors(snapshot.validation_errors_json),
      applyMode: resolveSnapshotApplyMode(env)
    });
  }
  ...
  if (request.method === "POST" && action === "apply") {
    const applyDenied = requireApplyDocumentSnapshots(session);
    ...
    try {
      ...
      const result = await applyDocumentSnapshot(env, id, session, {
        ...
        confirmedReviewCount: form.get("confirmedReviewCount"),
        confirmReview: form.get("confirmReview"),
        confirmedExcludeCount: form.get("confirmedExcludeCount"),
        confirmExclude: form.get("confirmExclude")
      });
      if (!result.ok) {
        const snapshot = await getDocumentSnapshot(env, id);
        if (!snapshot) return notFoundPage(session);
  ...
  if (request.method === "POST" && action === "cancel") {
    const result = await cancelDocumentSnapshot(env, id, session);
    if (!result.ok) return jsonResponse(result, { status: statusForSnapshotError(result) });
    return redirect(`/document-snapshots/${id}`);
  }
