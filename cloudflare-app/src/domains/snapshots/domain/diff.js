...
export function buildCanonicalValues({
  ...
  return {
    ...
    zoneNumber: nullableNumber(zoneNumber),
    rackNumber: nullableNumber(rackNumber),
...
export function computeChangeFlags({ action, beforeValues, afterValues, changedFields = [] }) {
  if (action === "create") {
    /** @type {string[]} */
    const flags = [CHANGE_FLAGS.CREATE];
    // 신규 행의 최종 상태가 disposed이면 CREATE + DISPOSE로 분류해 폐기 권한을 요구한다.
    if (afterValues?.status === "disposed") flags.push(CHANGE_FLAGS.DISPOSE);
    return flags;
  }
