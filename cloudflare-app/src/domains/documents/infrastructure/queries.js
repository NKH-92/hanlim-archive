import { collectDocumentFieldErrors, validateDocumentRecordFields, validateDocumentTextFields } from "../domain/validation.js";
import { MAX_DOCUMENT_TAGS } from "../domain/limits.js";
...
import { DATA_QUALITY_ISSUES } from "../../dataQuality/index.js";
import { getActiveCategories, getActiveTags, getCategories, getTags } from "../../masters/index.js";
import { getSlotOptions } from "../../racks/index.js";
import { buildDocumentFilterWhere } from "./filters.js";
...
export async function findDocumentsByNumbers(env, numbers) {
  if (!numbers.length) return { documents: [], missing: [] };
  ...
  const matched = new Set(documents.map((document) => String(document.document_number).toUpperCase()));
  return { documents, missing: numbers.filter((number) => !matched.has(number.toUpperCase())) };
}
