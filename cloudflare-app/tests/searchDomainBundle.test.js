...
test("Worker 엔트리포인트는 검색 유지보수의 세 고수준 계약만 사용한다", async () => {
  const worker = await readFile(new URL("../src/index.js", import.meta.url), "utf8");
  const domain = await readFile(new URL("../src/domains/search/index.js", import.meta.url), "utf8");
  ...
  assert.doesNotMatch(worker, /\bprocessSearchOutbox\b|\brebuildSearchIndexChunk\b/);
  assert.doesNotMatch(domain, /createSearchService|application\/service/);
});
