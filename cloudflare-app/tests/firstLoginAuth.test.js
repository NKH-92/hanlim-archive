...
test("0044 migration은 기존 2단계 인증 seed와 복구 코드를 모두 제거한다", async () => {
  const database = new DatabaseSync(":memory:");
...
test("로그인은 2단계 인증 등록 이력과 무관하게 세션을 발급하고 관련 경로를 노출하지 않는다", async () => {
  ...
  try {
    ...
    const login = await worker.fetch(
      loginRequest("admin-with-history@hanlim.com", password, "/app"),
      env
    );
    ...
    const settings = await worker.fetch(authenticatedRequest("/account/mfa", cookie), env);
    assert.equal(settings.status, 404);
    const challenge = await worker.fetch(authenticatedRequest("/login/mfa", cookie), env);
