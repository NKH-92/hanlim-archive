  ...
  const env = recordingEnv({
    first(sql) {
      if (sql.includes("SELECT * FROM categories")) {
        return { id: 2, name: "기존", description: "이전", sort_order: 1, is_active: 1, row_version: 1 };
      }
      return null;
    }
  });
...
test("랙 수정은 시스템 감사·랙 UPDATE·슬롯 동기화를 한 batch로 원자 처리한다", async () => {
  const env = recordingEnv({
    first(sql) {
  ...
  assert.match(env.state.batches[0][2].sql, /STALE_VERSION|abs\(-9223372036854775808\)/);
  assert.match(env.state.batches[0][3].sql, /UPDATE rack_slots/);
  assert.match(env.state.batches[0][4].sql, /INSERT INTO rack_slots/);
});
