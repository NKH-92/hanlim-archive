...
test("초기 관리자 migration은 최초 비밀번호 변경 상태를 만들 수 있다", async () => {
...
test("알려진 bootstrap credential은 최종 schema에서 격리되고 verifier가 폐기된다", async () => {
  const database = new DatabaseSync(":memory:");
  try {
...
test("DB guard는 epoch를 모르는 이전 Worker의 재활성화를 차단한다", async () => {
  const database = new DatabaseSync(":memory:");
  try {
    await applyMigrations(database);
    const inserted = database.prepare(`
    ...
    database.prepare(`
      UPDATE app_users SET status = 'approved', session_epoch = session_epoch + 1 WHERE id = ?
    `).run(userId);
    assert.deepEqual(
...
test("bootstrap migration은 기존 동명 계정의 비밀번호를 덮어쓰지 않는다", async () => {
  const database = new DatabaseSync(":memory:");
  try {
...
test("bootstrap 승격 이력이 있는 동명 계정은 검토 대상으로 남고 Admin으로 복구되지 않는다", async () => {
  const database = new DatabaseSync(":memory:");
  try {
    await applyMigrations(database, 26);
    database.prepare(`
...
test("동명 비특권 User는 Admin으로 자동 승격되지 않는다", async () => {
  const database = new DatabaseSync(":memory:");
  try {
    await applyMigrations(database, 26);
    database.prepare(`
      INSERT INTO app_users (
        ...
        CURRENT_TIMESTAMP, 'ops-reviewer', 0, 0, 0, 0, 0, 0, 0
      )
    `).run(INITIAL_EMAIL);
    await applyMigrationsFrom(database, 27, 34);
    const account = database.prepare(`
...
test("정당한 동명 Admin도 bootstrap remediation 이후 상태만 보고 자동 복구하지 않는다", async () => {
  const database = new DatabaseSync(":memory:");
  try {
    await applyMigrations(database, 26);
    database.prepare(`
      INSERT INTO app_users (
        ...
        CURRENT_TIMESTAMP, 'ops-reviewer', 1, 1, 1, 1, 1, 1, 1
      )
    `).run(INITIAL_EMAIL);
    await applyMigrationsFrom(database, 27, 34);
    const account = database.prepare(`
...
test("security_review_required 계정은 비밀번호가 맞아도 로그인·세션이 거부된다", async () => {
  const database = new DatabaseSync(":memory:");
  try {
    await applyMigrations(database);
    const { createPasswordRecord } = await import("../src/auth/passwords.js");
    ...
    const loginResponse = await worker.fetch(loginRequest("review-locked@hanlim.com", "review-pass-2026", "/app"), env);
...
test("최초 비밀번호 변경 대상은 returnUrl을 무시하고 변경 완료 전 업무 접근을 차단한다", async () => {
  const database = new DatabaseSync(":memory:");
  try {
    ...
    assert.equal((await worker.fetch(authenticatedRequest("/app", refreshedCookie), env)).status, 200);
  } finally {
    database.close();
  }
});
...
test("공개 가입 화면은 비활성화되고 등록되지 않은 이메일은 로그인할 수 없다", async () => {
  ...
  try {
    await applyMigrations(database);
    const env = sqliteEnv(database);
    assert.equal((await worker.fetch(new Request("https://archive.example.com/signup"), env)).status, 404);
...
test("MFA 제거 migration은 기존 seed와 복구 코드를 모두 제거한다", async () => {
  const database = new DatabaseSync(":memory:");
  try {
    await applyMigrations(database, 43);
    const inserted = database.prepare(`
      INSERT INTO app_users (
    ...
    `).run("former-mfa@hanlim.com", "기존 인증 사용자", "salt", "hash");
    const userId = Number(inserted.lastInsertRowid);
    database.prepare(`
      INSERT INTO user_mfa (user_id, status, encrypted_secret, enabled_at)
      VALUES (?, 'enabled', 'encrypted-seed', CURRENT_TIMESTAMP)
    `).run(userId);
    database.prepare(`
      INSERT INTO user_mfa_recovery_codes (user_id, code_digest)
      VALUES (?, 'recovery-digest')
    `).run(userId);
    await applyMigrationsFrom(database, 44, 44);
    assert.equal(database.prepare("SELECT COUNT(*) AS count FROM user_mfa").get().count, 0);
    assert.equal(database.prepare("SELECT COUNT(*) AS count FROM user_mfa_recovery_codes").get().count, 0);
  } finally {
    database.close();
  }
});

test("최종 schema는 제거된 MFA 저장소 없이 로그인하고 MFA route를 노출하지 않는다", async () => {
  const database = new DatabaseSync(":memory:");
  try {
    await applyMigrations(database);
    const { createPasswordRecord } = await import("../src/auth/passwords.js");
    ...
    const env = sqliteEnv(database);
    const login = await worker.fetch(loginRequest("admin-with-history@hanlim.com", password, "/app"), env);
    assert.equal(login.status, 302);
    assert.equal(login.headers.get("Location"), "/app");
    const cookie = login.headers.get("Set-Cookie");
    assert.match(cookie, /^hanlim_session=/);
    const app = await worker.fetch(authenticatedRequest("/app", cookie), env);
    assert.equal(app.status, 200);
    assert.doesNotMatch(await app.text(), /2단계 인증|\/account\/mfa/);
    assert.equal((await worker.fetch(authenticatedRequest("/account/mfa", cookie), env)).status, 404);
    assert.equal((await worker.fetch(authenticatedRequest("/login/mfa", cookie), env)).status, 404);
  } finally {
    database.close();
  }
});
